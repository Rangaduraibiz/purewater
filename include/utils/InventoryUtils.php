...
function deleteInventoryProductDetails1($focus) {
	global  $adb;
	$adb->pquery("delete from vtiger_inventoryproductrel_other where id=?", array($focus->id));
}

function deleteInventoryProductDetails2($focus) {
	global  $adb;
	$adb->pquery("delete from vtiger_inventoryproductrel_other_masn where id=?", array($focus->id));
}

function deleteEquipmentLines($focus , $table) {
	global  $adb;
	$adb->pquery("delete from $table where id=?", array($focus->id));
}

function deleteInventoryProductDetails2Mobile($id) {
	global  $adb;
	$adb->pquery("delete from vtiger_inventoryproductrel_other_masn where id=?", array($id));
}
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	if($focus->mode == 'edit')
	{
		...
		$NODELETEOFLINEITEMS = vglobal('NODELETEOFLINEITEMS');
		if($NODELETEOFLINEITEMS == true){
		} else {
	...
	if (in_array($module,getLineITemGeneralisedModules())) {
		...
		$sql = "SELECT * FROM `vtiger_field` LEFT JOIN vtiger_blocks
         	...
         	and helpinfo LIKE 'li_lg%' and blocklabel = ? and tablename = 'vtiger_inventoryproductrel'
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
	    ...
	    $prod_name = getProductName($prod_id);
		...
		if(isset($_REQUEST['dependent'.$i])){
			$dependent = vtlib_purify($_REQUEST['dependent'.$i]);
		} else {
			$dependent = 'off';
		}
		...
		if (in_array($module,getLineITemGeneralisedModules())) {
			...
			if (empty($fieldNames)) {
				$query = "INSERT INTO vtiger_inventoryproductrel(id, productid, productname, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, dependency )
						VALUES(?,?,?,?,?,?,?,?,?,?,?)";
				$qparams = array($focus->id, $prod_id, $prod_name, $prod_seq, $qty, $listprice, $comment, $description, $purchaseCost, $margin, $dependent);
			} else {
				$query = "INSERT INTO vtiger_inventoryproductrel(id, productid, productname, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, dependency,". implode(',',$fieldNames).")
...
function saveInventoryFailedProductDetails(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '') {
	global $log, $adb;
	$id = $focus->id;
	...
	foreach ($fieldNamesData as $fieldnameKey => $fieldinfo) {
	...
	for ($i = 1; $i <= $tot_no_prod; $i++) {
		...
		$purchaseCost = vtlib_purify($_REQUEST['purchaseCost' . $i]);
		$margin = vtlib_purify($_REQUEST['margin' . $i]);
		$igLineItemId = vtlib_purify($_REQUEST['igLineItemId' . $i]);
		...
		if (in_array($module, getLineITemGeneralisedModules())) {
			$fieldNameValues = [];
			$updatequery = "update vtiger_inventoryproductrel set ";
			foreach ($modifiedFieldsArray as $fieldNamesdata) {
				$valueOfTheField = vtlib_purify($_REQUEST[$fieldNamesdata['fieldname'] . $i]);
				if ($fieldNamesdata['uitype'] == '56') {
					if ($valueOfTheField == 'on') {
						$valueOfTheField = 1;
				...
				$updatequery .=  $fieldNamesdata['fieldname']. " = ? , ";
				if ($fieldNamesdata['uitype'] == '7' &&  ($valueOfTheField == null || $valueOfTheField == "")) {
					$valueOfTheField = NULL;
				}
				array_push($fieldNameValues, $valueOfTheField);
			}
			$query = rtrim($updatequery, ' , ');
			$query = $query . ' Where lineitem_id = ? ';
			array_push($fieldNameValues, $igLineItemId);

			if (empty($igLineItemId)) {
				$query = "INSERT INTO vtiger_inventoryproductrel(id, productid, productname, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, dependency )
						VALUES(?,?,?,?,?,?,?,?,?,?,?)";
				$qparams = array($focus->id, $prod_id, $prod_name, $prod_seq, $qty, $listprice, $comment, $description, $purchaseCost, $margin, $dependent);
			} else if (!empty($modifiedFieldNamesArray) && !empty($igLineItemId)) {
				$qparams = $fieldNameValues;
			}
		} else {
			$query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, productname, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, dependency)
					VALUES(?,?,?,?,?,?,?,?,?,?,?)';
...
function saveInventoryProductDetails1(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '') {
	global $log, $adb;
	$log->debug("Entering into function saveInventoryProductDetails($module).");
	if (isset($_REQUEST['convert_from']) && $_REQUEST['convert_from'] != '') {
		$id = vtlib_purify($_REQUEST['return_id']);
	...
	if ($focus->mode == 'edit') {
		deleteInventoryProductDetails1($focus);
	}
	$tot_no_prod = $_REQUEST['totalProductCount1'];
	$prod_seq = 1;
	global $current_user;
	$formate = $current_user->date_format;
	$fieldNames = [];
	...
	for ($i = 1; $i <= $tot_no_prod; $i++) {
		...
		$prod_id = vtlib_purify($_REQUEST['hdnProductId_other' . $i]);
		$prod_name = getProductName($prod_id);
		if (!$prod_id) {
			$prod_name = '';
		}
		if (isset($_REQUEST['productDescription' . $i]))
			$description = vtlib_purify($_REQUEST['productDescription' . $i]);
		$qty = vtlib_purify($_REQUEST['qty_other' . $i]);
		$listprice = vtlib_purify($_REQUEST['listPrice' . $i]);
		$comment = vtlib_purify($_REQUEST['comment' . $i]);
		$purchaseCost = vtlib_purify($_REQUEST['purchaseCost' . $i]);
		$margin = vtlib_purify($_REQUEST['margin' . $i]);
		...
		if ($module == 'FailedParts' || $module == 'ServiceReports') {
			...
			foreach ($fieldNamesData as $fieldNamesdata) {
				...
				// array_push($fieldNameValues, $valueOfTheField);
				$fieldNameValues[$fieldNamesdata['fieldname']] = $valueOfTheField;
			}

			if ($focus->column_fields['sr_ticket_type'] == 'PERIODICAL MAINTENANCE') {
				include_once('include/utils/GeneralUtils.php');
				if ($fieldNameValues['sad_valid_sl_no'] == "Yes") {
					$record = getLastPeriodicMaintainenceDetails($focus->column_fields['equipment_id'], $fieldNameValues['sad_sel_ag_name']);
					if (!empty($record)) {
						$fieldNameValues['sad_ag_sl_no'] = $record[0]['sad_ag_sl_no'];
						$fieldNameValues['sad_dof'] = $record[0]['sad_dof'];
						$fieldNameValues['sad_manu_name'] = $record[0]['sad_manu_name'];
						$lastHMR = floatval($record[0]['hmr']);
						$currentHMR = floatval($focus->column_fields['hmr']);
						$validation = $fieldNameValues['sad_valid_sl_no'];
						$lastWorkedHours = floatval($record[0]['sad_whoa']);
						$fieldNameValues['sad_whoa'] = getWorkedHors($lastHMR, $currentHMR, $validation, $lastWorkedHours);
					}
				} else {
					$record = getLastPeriodicMaintainenceDetails($focus->column_fields['equipment_id'], $fieldNameValues['sad_sel_ag_name']);
					if (!empty($record)) {
			...
			$fieldNamesFromOb = array_keys($fieldNameValues);
			$fieldNameValuesFromOb = array_values($fieldNameValues);
			if (empty($fieldNamesFromOb)) {
				$query = "INSERT INTO vtiger_inventoryproductrel_other(id, productid, productname, sequence_no, quantity)
					VALUES(?,?,?,?,?)";
			} else {
				$query = "INSERT INTO vtiger_inventoryproductrel_other(id, productid, productname, sequence_no, quantity, " . implode(',', $fieldNamesFromOb) . ")
					VALUES(?,?,?,?,?," . generateQuestionMarks($fieldNamesFromOb) . ")";
			}
			$qparams = array($focus->id, $prod_id, $prod_name, $prod_seq, $qty);
			$qparams = array_merge($qparams, $fieldNameValuesFromOb);
		} else {
			$query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, productname, sequence_no, quantity, listprice, comment, description, purchase_cost, margin, dependency)
					VALUES(?,?,?,?,?,?,?,?,?,?,?)';
			$qparams = array($focus->id, $prod_id, $prod_name, $prod_seq, $qty, $listprice, $comment, $description, $purchaseCost, $margin, $dependent);
...
function saveInventoryProductDetails2(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '') {
	global $log, $adb;

	if ($focus->mode == 'edit') {
		deleteInventoryProductDetails2($focus);
	}
	$tot_no_prod = $_REQUEST['totalProductCount2'];
	$prod_seq = 1;
	global $current_user;
	$formate = $current_user->date_format;
	$fieldNames = [];
	...
	for ($i = 1; $i <= $tot_no_prod; $i++) {
		...
		if ($module == 'ServiceReports') {
			...
			if (empty($fieldNames)) {
				$query = "INSERT INTO vtiger_inventoryproductrel_other_masn(id, productid, productname, sequence_no, quantity)
					VALUES(?,?,?,?,?)";
			} else {
				$query = "INSERT INTO vtiger_inventoryproductrel_other_masn(id, productid, productname, sequence_no, quantity, " . implode(',', $fieldNames) . ")
					VALUES(?,?,?,?,?," . generateQuestionMarks($fieldNames) . ")";
			}
			$qparams = array($focus->id, $prod_id, $prod_name, $prod_seq, $qty);
			$qparams = array_merge($qparams, $fieldNameValues);
...
function saveLineDetailsEquipment(&$focus, $module) {
	global $log, $adb;

	$table = 'vtiger_inventoryproductrel_equipment';
	if ($focus->mode == 'edit') {
		deleteEquipmentLines($focus, $table);
	}
	$tot_no_prod = $focus->column_fields['total_year_cont'];
	$prod_seq = 1;
	global $current_user;
	$formate = $current_user->date_format;
	$fieldNames = [];
...
function saveInventoryProductDetailsFromMobile($LineItemsOther, $module, $createdId, $recordValues) {
	$createdId = explode('x', $createdId);
	...
	foreach ($LineItemsOther as $lineItem) {
		$fieldNameValues = [];
		$prod_id = '';
		$prod_name = '';
		$qty = '';
		$prod_seq = 1;
		foreach ($fieldNamesData as $fieldNamesdata) {
			$valueOfTheField = vtlib_purify($lineItem[$fieldNamesdata['fieldname']]);
			if ($fieldNamesdata['uitype'] == '56') {
				if ($valueOfTheField == 'on') {
					$valueOfTheField = 1;
				} else {
		...
		if ($recordValues['sr_ticket_type'] == 'PERIODICAL MAINTENANCE') {
			include_once('include/utils/GeneralUtils.php');
			if ($fieldNameValues['sad_valid_sl_no'] == "Yes") {
				$record = getLastPeriodicMaintainenceDetails($focus->column_fields['equipment_id'], $fieldNameValues['sad_sel_ag_name']);
				if (!empty($record)) {
		...
		$prod_id = $lineItem['productid'];
		if (strpos($prod_id, 'x')) {
			$prod_id = explode('x', $prod_id)[1];
		}
		$prod_name = getProductName($prod_id);
		$qty = $lineItem['qty_other'];
		if (empty($fieldNames)) {
			$query = "INSERT INTO vtiger_inventoryproductrel_other(id, productid, productname, sequence_no, quantity)
					VALUES(?,?,?,?,?)";
		} else {
			$query = "INSERT INTO vtiger_inventoryproductrel_other(id, productid, productname, sequence_no, quantity, " . implode(',', $fieldNames) . ")
					VALUES(?,?,?,?,?," . generateQuestionMarks($fieldNames) . ")";
		}
		$qparams = array($createdId, $prod_id, $prod_name, $prod_seq, $qty);
		$qparams = array_merge($qparams, $fieldNameValues);
		$adb->pquery($query, $qparams);
		$prod_seq = $prod_seq + 1;
	}
...
function saveInventoryProductDetailsFromMobileMASN($LineItemsOther, $module, $createdId) {
	$createdId = explode('x', $createdId);
	$createdId = $createdId[1];
	$fieldNames = [];
	$fieldNamesData = [];
	...
	$formate = $current_user->date_format;
	deleteInventoryProductDetails2Mobile($createdId);
	if ($module == 'FailedParts' || $module == 'ServiceReports') {
		$tabId = getTabId($module);
		$sql = "SELECT * FROM `vtiger_field` LEFT JOIN vtiger_blocks
         on vtiger_blocks.blockid = vtiger_field.block where vtiger_field.tabid = ? 
...
function deleteContractsAvalibiltyValues($id) {
	global  $adb;
	$adb->pquery("delete from vtiger_inventoryproductrel_equipment where id=?", array($id));
}

function saveContractsAvalibiltyValues($LineItemsOther, $module, $createdId) {
	$createdId = explode('x', $createdId);
	$createdId = $createdId[1];
	$fieldNames = [];
	$fieldNamesData = [];
	global $current_user,$adb;
	$table = 'vtiger_inventoryproductrel_equipment';
	deleteContractsAvalibiltyValues($createdId);
	if ($module == 'Equipment') {
		$tabId = getTabId($module);
		$sql = "SELECT * FROM `vtiger_field` LEFT JOIN vtiger_blocks
		 on vtiger_blocks.blockid = vtiger_field.block where vtiger_field.tabid = ? 
...
function updatSecondaryLineItemFields($LineItemsOther, $module, $createdId) {
	$createdId = explode('x', $createdId);
	$createdId = $createdId[1];
	$fieldNames = [];
	$fieldNamesData = [];
	global $adb;
	global $current_user;
	if ($module == 'ReturnSaleOrders') {
		$tabId = getTabId($module);
		$sql = "SELECT * FROM `vtiger_field` LEFT JOIN vtiger_blocks
         on vtiger_blocks.blockid = vtiger_field.block where vtiger_field.tabid = ? 
         and helpinfo = 'li_lg' and blocklabel = ? 
		 and tablename = 'vtiger_inventoryproductrel_fp'
		 ORDER BY `vtiger_field`.`sequence` ASC;";
		$result = $adb->pquery($sql, array($tabId, 'LBL_ITEM_DETAILS'));
		while ($row = $adb->fetch_array($result)) {
			array_push($fieldNames, $row['fieldname']);
	...
	foreach ($LineItemsOther as $lineItem) {
		$fieldNameValues = [];
		$updatequery = "update vtiger_inventoryproductrel_fp set ";
		foreach ($fieldNamesData as $fieldNamesdata) {
			$valueOfTheField = vtlib_purify($lineItem[$fieldNamesdata['fieldname']]);
			if ($fieldNamesdata['uitype'] == '56') {
				if ($valueOfTheField == 'on') {
		...
		if (!empty($fieldNames)) {
			array_push($fieldNameValues , $lineItem['lineitem_id']);
			$adb->pquery($query, $fieldNameValues);
		}
	}
}

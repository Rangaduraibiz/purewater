...
class VtigerModuleOperation extends WebserviceEntityOperation {
	...
	public function query($q){
		...
		$output = array();
		$entityName = $this->webserviceObject->getEntityName();
		if ($entityName == 'Equipment') {
			for ($i = 0; $i < $noofrows; $i++) {
				$row = $this->pearDB->fetchByAssoc($result, $i);
				if (!$meta->hasPermission(EntityMeta::$RETRIEVE, '38x'.$row[$tableIdColumn])) {
					continue;
				}
	...
	function getDescribeFieldArray($webserviceField){
		...
		if($webserviceField->getFieldName()!='employee_role' && $typeDetails['name'] === "reference") {
			$typeDetails['refersTo'] = $webserviceField->getReferenceList();
		}
		if($webserviceField->getFieldName()=='employee_role'){
			$typeDetails['name'] = 'picklist';

...
class CRMEntity {
	 ...
	 * return void
	 */
	function uploadAndSaveFile($id, $module, $file_details, $attachmentType='Attachment', $fieldNameOfAttach ='') {
		global $log;
		...
		if ($save_file == 'true' && $upload_status == 'true') {
			...
			$params2 = array($current_id, $filename, $this->column_fields['description'], $filetype, $upload_file_path, $encryptFileName, $fieldNameOfAttach);
						...
						if(count($IMG_FILES)){
							foreach($IMG_FILES as $fileIndex => $file) {
								if($file['error'] == 0 && $file['name'] != '' && $file['size'] > 0) {
									...
									$attachType = 'Image';
									if($module == 'HelpDesk' || $module == 'ServiceReports'){
										$attachType = 'Attachment';
									}
									$attachmentId = $this->uploadAndSaveFile($this->id,$module,$file,$attachType,$fieldname);
									if($attachmentId) {
...
//Added by Richie
	...
	function get_related_list($id, $cur_tab_id, $rel_tab_id, $actions = false) {
		...
		// $data = getRoleIdOfcustomer($id);
		// $roles = $data[0];
		// $accountId = $data[1];
		// $belowRoleContactIds = getAllBelowRoleContacts($accountId,$roles);
		// array_push($belowRoleContactIds,$id);

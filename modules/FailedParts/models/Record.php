...
class FailedParts_Record_Model extends Inventory_Record_Model {
    ...
    /* function getCreateSalesOrderUrl() {
        ...
        return "index.php?module=".$salesOrderModuleModel->getName()."&view=".$salesOrderModuleModel->getEditViewName()."&failedparts_id=".$this->getId().
    ...
    function getProductsForSo() {
        $numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
        $relatedProducts = $this->getAssociatedProductsSO($this->getModuleName(), $this->getEntity());
    ...
    function getAssociatedProductsSO($module, $focus, $seid = '', $refModuleName = false) {
        global $log;
        ...
        for ($i = 1; $i <= $num_rows; $i++) {
            ...
            $qty = $adb->query_result($result, $i - 1, 'so_creatable_qty');
            ...
            $product_Detail[$i]['igLineItemId' . $i] = $adb->query_result($result, $i - 1, 'lineitem_id');
            ...
            if ($module == 'Vendors' || $module == 'Products' || $module == 'Services' || in_array($module, $lineItemSupportedModules)) {
                ...
                $product_Detail[$i]['part_description'.$i]= $productdescription;
            } else {
                $product_Detail[$i]['comment' . $i] = $comment;
                $product_Detail[$i]['part_description'.$i]= $comment;
            }
            ...
            $product_Detail[$i]['failedpart_lineid' . $i] = $adb->query_result($result, $i - 1, 'lineitem_id');
    ...
    function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false) {
        ...
        if (in_array($module, $inventoryModules)) {
            $query = "SELECT
					...
					LEFT JOIN vtiger_products ON vtiger_products.productid=vtiger_inventoryproductrel.productid
					LEFT JOIN vtiger_service ON vtiger_service.serviceid=vtiger_inventoryproductrel.productid
					WHERE id=? and vtiger_inventoryproductrel.fail_pa_pa_status != 'Closed' ORDER BY sequence_no";
            $params = array($focus->id);
        } elseif (in_array($module, $lineItemSupportedModules)) {
            $query = '(SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.product_no AS productcode, vtiger_products.purchase_cost,
					vtiger_products.unit_price, vtiger_products.qtyinstock, vtiger_crmentity.deleted, "Products" AS entitytype,
        ...
        for ($i = 1; $i <= $num_rows; $i++) {
            ...
            if (in_array($module, getLineITemGeneralisedModules())) {
                foreach ($fieldNames as $fieldName) {
                    $product_Detail[$i][$fieldName . $i] = decode_html(from_html($adb->query_result($result, $i - 1, $fieldName)));
                }
            }
    ...
    function getAssociatedProductsForDetail($module, $focus, $seid = '', $refModuleName = false) {
        global $log;
        $log->debug("Entering getAssociatedProducts(" . $module . "," . get_class($focus) . "," . $seid . "='') method ...");
        global $adb;
    ...
    function getProductsForDetail() {
        $numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
        $relatedProducts = $this->getAssociatedProductsForDetail($this->getModuleName(), $this->getEntity());
        $productsCount = count($relatedProducts);
        if(empty($productsCount)){
            return [];
        }
    ...
    public function IGisEditable($id) {
		global $adb;
		$sql = "SELECT 1 FROM `vtiger_inventoryproductrel` 
        where id = ? and fail_pa_pa_status != 'Closed'";
    ...
    public function IGisSOCreatable($id) {
		global $adb;
		$sql = "SELECT 1 FROM `vtiger_inventoryproductrel` 
        where id = ? and fail_pa_pa_status = 'Validation Pending'";
		$sqlResult = $adb->pquery($sql, array($id));
		$num_rows = $adb->num_rows($sqlResult);
		if ($num_rows > 0) {
			return true;

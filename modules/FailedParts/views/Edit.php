...
class FailedParts_Edit_View extends Inventory_Edit_View {
    ...
    public function process(Vtiger_Request $request) {
        ...
        if ($request->has('totalProductCount')) {
            ...
            $recordModel = FailedParts_Record_Model::getInstanceById($record, $moduleName);
        ...
        $sql = "SELECT * FROM `vtiger_field` where tabid = ? and helpinfo = 'li_lg' and presence != 1
        ORDER BY `vtiger_field`.`sequence` ASC";
        $result = $adb->pquery($sql, array($tabId));
        $fields = [];
        $fieldNames = [];
        $dependentList = array('excluded_qty_rem');
        $excludedFields = array(
            'pending_days',
            'tot_excluded_qty_rem', 'total_excluded_qty',
            'salesorder_cr_qty','so_creatable_qty','submitted_qty_log'
        );
        while ($row = $adb->fetch_array($result)) {
            ...
            if (in_array($fldName, $dependentList)) {
        ...
        if ($request->get('displayMode') == 'overlay') {
            ...
            $viewer->view('EditView.tpl', 'FailedParts');
        }
    }
}

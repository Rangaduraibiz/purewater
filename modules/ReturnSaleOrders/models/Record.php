...
class ReturnSaleOrders_Record_Model extends Inventory_Record_Model {
    ...
    function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false) {
        ...
        if (in_array($module, $inventoryModules)) {
            $query = "SELECT
					...
					vtiger_inventoryproductrel.description AS product_description, vtiger_inventoryproductrel.*,vtiger_inventoryproductrel_fp.*,
					vtiger_crmentity.deleted FROM vtiger_inventoryproductrel
                    LEFT JOIN vtiger_inventoryproductrel_fp ON vtiger_inventoryproductrel_fp.id=vtiger_inventoryproductrel.id

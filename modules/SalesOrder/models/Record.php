...
class SalesOrder_Record_Model extends Inventory_Record_Model {
	...
	public function getAssociatedProductsSTOEnabled($module, $focus, $seid = '', $refModuleName = false, $LineItemId) {
        global $log;
        ...
        if (in_array($module, $inventoryModules)) {
            $query = "SELECT
					...
					WHERE id=? and lineitem_id = ? ORDER BY sequence_no";
            $params = array($focus->id, $LineItemId);
        ...
        for ($i = 1; $i <= $num_rows; $i++) {
            ...
            if ($module == 'Vendors' || $module == 'Products' || $module == 'Services' || in_array($module, $lineItemSupportedModules)) {
                ...
                if(empty($comment)){
                    $product_Detail[$i]['comment' . $i] = $adb->query_result($result, $i - 1, 'part_description');
                }
            }
    ...
    public function getProductsForSTO($LineItemId) {
        $numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
        $relatedProducts = $this->getAssociatedProductsSTOEnabled($this->getModuleName(), $this->getEntity(), '',false,$LineItemId);

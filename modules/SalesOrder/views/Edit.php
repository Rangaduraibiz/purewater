...
class SalesOrder_Edit_View extends Inventory_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if ($request->has('totalProductCount')) {
			...
			$parentRecordModel = FailedParts_Record_Model::getInstanceById($referenceId);
			$currencyInfo = $parentRecordModel->getCurrencyInfo();
			$taxes = $parentRecordModel->getProductTaxes();
		...
		and helpinfo = 'li_lg' and presence != 1 ORDER BY `vtiger_field`.`sequence` ASC";
		$result = $adb->pquery($sql, array($tabId));
		$fields = [];
		$fieldNames = [];
		$specialFieldsHidden = ['failedpart_lineid'];
		while ($row = $adb->fetch_array($result)) {
			if ($row['fieldname'] == 'rso_created_qty') {
				continue;
			}
			if ($row['uitype'] == '16') {
			...
			if ($row['fieldname'] == 'failedpart_lineid') {
				$row['hiddenSpecial'] = true;
			}
			$row['isMandatory'] = $this->isMandatory($row['typeofdata']);
	...
	function isMandatory($typeofdata) {

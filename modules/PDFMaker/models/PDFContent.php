...
class PDFMaker_PDFContent_Model extends PDFMaker_PDFContentUtils_Model {
    ...
    function __construct($l_module, $l_focus, $l_language,$tempid){
    ...
    private function getTemplateData(){
        ...
        $result = self::$db->pquery("SELECT vtiger_pdfmaker.*, vtiger_pdfmaker_settings.* FROM vtiger_pdfmaker "
    ...
    private function getInventoryProducts($module, $focus){
        if (!empty($focus->id)) {
            ...
            foreach ($relatedProducts AS $i => $PData) {
                ...
                $serialNumber = $PData["serialNumber".$sequence];

                if ($serialNumber != "") {
                    if( strpos($serialNumber,'&lt;br /&gt;') === false && strpos($serialNumber,'&lt;br/&gt;') === false && strpos($comment,'&lt;br&gt;') === false) {
                        $serialNumber = str_replace("\\n", "<br>", nl2br($serialNumber));
                    }
                    $serialNumber = html_entity_decode($serialNumber, ENT_QUOTES, self::$def_charset);
                }
                ...
                $Details["P"][$i]["PRODUCT_SERIALNUMBER"] = $serialNumber;
                ...
                if ($taxtype == "individual"){
                    ...
                    for ($tax_count = 0; $tax_count < count($tax_details); $tax_count++){
                        ...
                        $kilmat[$tax_name] = (float)$kilmat[$tax_name] + $individual_taxamount;
                        if ($tax_name != ""){
            ...
            $Details["TOTAL"]["PRODUCT_TAX1"] = round($kilmat['tax1'],2);
            $Details["TOTAL"]["PRODUCT_TAX2"] = round($kilmat['tax2'],2);
            $Details["TOTAL"]["PRODUCT_TAX3"] = round($kilmat['tax3'],2);
        }
        ...
        if (is_array($html->find("td"))){
            foreach ($html->find("td") as $td){
                if (trim($td->plaintext) == "#PRODUCTBLOC_".$block_type."START#"){
                    ...
                    $tableDOM["subtotal"] = '';
                    $tableDOM["colspan"] = $td->getAttribute("colspan") - 1;
                }
    ...
    private function convertInventoryModules() {
        ...
        if ($num_rows > 0) {
            ...
            $var_array = array();
            $Blocks = array('', 'PRODUCTS_', 'SERVICES_', 'UNIQUE_');
            foreach ($Blocks as $block_type) {
                if (strpos(self::$content, "#PRODUCTBLOC_" . $block_type . "START#") !== false && strpos(self::$content, "#PRODUCTBLOC_" . $block_type . "END#") !== false) {
                    ...
                    $breakline_type = "";
                    $colspan = '';
                    $colspan = $tableTag["colspan"];
                    if (count($breaklines) > 0) {
                        if ($tableTag !== false) {
                            if ($show_subtotal == 1) {
                                $breakline_type = $tableTag["subtotal"];
                            } else {
                                $breakline_type = $tableTag["footer"];
                            }
                            $breakline_type.= "</table>" . $tableTag["tag"];
//                            $breakline_type.= "</table>" . $tableTag["tag"];
                            if ($show_header == 1) {
                    ...
                    if ($Products["P"]) {
                        $userProductIds = [];
                        $key = 'PRODUCT_SECTION_NAME';
                        $result = array();
                        foreach ($Products["P"] as $val) {
                            if (array_key_exists($key, $val)) {
                                $productID = $val['PRODUCTS_CRMID'];
                                $result[$val[$key]][] = $val;
                                if(isset($sectionTotalArray[$val[$key]])){
                                    $a = (double)filter_var($sectionTotalArray[$val[$key]], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                    $b = (double)filter_var($val['PRODUCTTOTALSUM'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                    $sectionTotalArray[$val[$key]] = $a +  $b;
                                } else {
                                    $sectionTotalArray[$val[$key]] = (double)filter_var($val['PRODUCTTOTALSUM'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
                                }
                            } else {
                                $result[""][] = $val;
                            }
                        }
                        $totalSections = count($result);
                        $imata = 0;
                        foreach ($result as $keyda => $valueda) {
                            $lmnk = 0;
                            $igata = count($valueda);
                            foreach ($valueda as $Product_Details) {
                                $productId = $Product_Details['RECORD_ID'];
                                ...
                                foreach ($ProductParts as $productpartid => $productparttext) {
                                    if ('UNIQUE_' == $block_type && in_array($productId, $userProductIds)) {
                                        $productparttext = '';
                                    }
                                    $userProductIds[] = $productId;
                                    $breakline = "";
                                    if (($lmnk+1) == $igata && !empty($keyda)) {
                                        $breakline = $breakline_type;
                                        $subtotal_tr = '<tr style="background-color:#39ac39;">';
                                        $subtotal_tr .= "<td colspan='" . ($colspan+1) . "' style='font-size:14px;padding:5px;color: white'>$keyda</td>";
                                        $subtotal_tr .= '<td style="font-size:14px;padding:1px; text-align: right;color: white"> '. $this->formatNumberToPDF($sectionTotalArray[$keyda]) .' &#x20B9; </td>';
                                        $subtotal_tr .= "</tr>";
                                        if($totalSections != ($imata+1)){
                                            $subtotal_tr .= "<tr><td colspan='" . ($colspan+2) . "'>&#160;</td></tr><tr><td colspan='" . ($colspan+2) . "'>&#160;</td></tr>";
                                        }
                                        $breakline = $breakline . $subtotal_tr;
                                    }
    ...
    private function replaceInventoryDetailsBlock($module, $focus, $is_related = false){
        ...
        self::$rep["$" . $prefix . "PRODUCT_TAX1$"] = $Products["TOTAL"]["PRODUCT_TAX1"];
        self::$rep["$" . $prefix . "PRODUCT_TAX2$"] = $Products["TOTAL"]["PRODUCT_TAX2"];
        self::$rep["$" . $prefix . "PRODUCT_TAX3$"] = $Products["TOTAL"]["PRODUCT_TAX3"];
    ...
    public function getservicerequestImage($id, $site_url) {
        ...
        if (isset($id) and $id != "") {
            ...
            while ($row = $db->fetchByAssoc($result)) {
                ...
                $image = "<img height=400 width=450 src='" . $site_url . "/" . $image_src . "'/>";
                $imagevalues[] = $image;
            }
            return $imagevalues;
        } else {
    ...
    public function getservicerequestSD($id) {
        ...
        if (isset($id) and $id != "") {
            ...
            $table = "<table><thead><tr><td>Sl No.</td><td>Part Number</td><td>Description</td><td>Quantity</td><td>Status</td><td>Events</td><td>Do not Show on Customer Service report</td></tr></thead><tbody>";
            while ($row = $db->fetchByAssoc($result)) {
                $trow = array();
                $trow = PDFMaker_Module_Model::fixStoredName($row);
                $table .="<tr><td>" . $trow["sequence_no"] . "</td><td>" . $trow["productname"] . "</td><td>" . $trow["comment"] . "</td><td>" . $trow["quantity"] . "</td><td>" .$trow["sad_line_status"] . "</td><td>" . $trow["sad_line_event"] ."</td><td>" . $trow["sad_line_dnsoc"] ."</td></tr>";

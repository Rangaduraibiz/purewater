		...
		if ($this->interactive) {
			...
			//do {
				...
				if (!$moduleInstance) {
					...
					$moduleDelete = new Vtiger_Module_Delete($module);
					$moduleDelete->deleteModule();
					echo "DONE.\n";
				}
			//} while (true);
		} else {
...
class Vtiger_Module_Delete {  
    ...
    public function __construct($moduleName) {
        ...
        $this->langDir = 'languages/en_us/' . $moduleName;
		$this->modulelayoutDir = 'layouts/v7/modules/' . $moduleName;
	...
	public function deletelayoutFiles() {
        $this->rrmdir($this->modulelayoutDir);
    }
    
    public function deleteTables() {
		global $adb;
        $sql = "DROP TABLE IF EXISTS vtiger_" . strtolower($this->moduleName) . ", vtiger_" . strtolower($this->moduleName) . "cf, vtiger_" . strtolower($this->moduleName) . "_seq";
		echo $sql;

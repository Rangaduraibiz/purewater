...
{strip}
	...
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
		...
		{if $BLOCK_LABEL_KEY eq 'Major_Aggregates_Sl_No'}
			{if $SERVICEREPORTTYPE eq 'PRE-DELIVERY'}
				{include file="LineItemsDetailShortDam1.tpl"|@vtemplate_path:'RecommissioningReports'}
			{/if}
		{else}
									...
									{else if $FIELD_MODEL->get('uitype') eq "69" || $FIELD_MODEL->get('uitype') eq "105"}
										...
										<td class="fieldValue {$WIDTHTYPE}">
											<ul id="imageContainer" class="imageSlider">
												...
												{/if} *}
												{if $MODULE_NAME eq 'HelpDesk' ||  $MODULE_NAME eq 'RecommissioningReports'}
														...
														{if $FIELD_MODEL->getName() eq $IMAGE_INFO.fieldNameFromDB}
															{if !empty($IMAGE_INFO.url) && !empty({$IMAGE_INFO.orgname})}
																{if ( strpos($IMAGE_INFO.name , '.png') >= 1 || strpos($IMAGE_INFO.name , '.jpeg') >= 1 || strpos($IMAGE_INFO.name , '.jpg') >= 1)  }
																	<li><span>
																		<a onclick="openInNewTab('{$IMAGE_INFO.location}');">{$IMAGE_INFO.name}</a></span></li>
																{else}
																	<li><img style="display:none" data-image-id="{$IMAGE_INFO.id}" target="_blank" title="{$IMAGE_INFO.name}"/><span><a onclick="openInNewTab('{$IMAGE_INFO.location}');">{$IMAGE_INFO.name}</a></span></li>
																{/if}
															{/if}
														{/if}
													{/foreach}

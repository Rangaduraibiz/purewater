{strip}
{foreach item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER}
...
{if $SERVICEREPORTTYPE|in_array:$PRODUCTEDITDISABLEDTYPES}
...
<input type="hidden" name="totalProductCount1" id="totalProductCount1" value="{$row_no}" />
...
<div name='editContent'>
	...
	{assign var=BLOCK_LABEL value=$BLOCKNAMEFROMDEPENDENCY}
	{if $BLOCK_FIELDS|@count gt 0}
		<div class='fieldBlockContainer' id="ShortagefieldBlockContainer" >
			...
			<div class="lineitemTableContainer">
				<table class="table table-bordered" id="lineItemTab1">
					<tr>
						<td><strong>{vtranslate('LBL_SLNO',$MODULE)}</strong></td>
						{if $IMAGE_EDITABLE}
							<td>
					...
					<tr id="anorow0" class="hide lineItemCloneCopy" data-row-num="0">
						{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'RecommissioningReports' row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
					</tr>
					{if count($RELATED_PRODUCTS_OTHER) gt 0}
						{foreach key=row_no item=data from=$RELATED_PRODUCTS_OTHER}
							<tr id="row{$row_no}" data-row-num="{$row_no}" class="lineItemRow" {if $data["entityType$row_no"] eq 'Products'}data-quantity-in-stock={$data["qtyInStock$row_no"]}{/if}>
								{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'RecommissioningReports' row_no=$row_no data=$data}
							</tr>
						{/foreach}
					{/if}
					{* {if count($RELATED_PRODUCTS_OTHER) eq 0 and ($PRODUCT_ACTIVE eq 'true' || $SERVICE_ACTIVE eq 'true')}
						<tr id="row1" class="lineItemRow" data-row-num="1">
							{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'RecommissioningReports' row_no=1 data=[] IGNORE_UI_REGISTRATION=false}
						</tr>
					{/if} *}
				</table>
			</div>
			...
			{if $PRODUCT_ACTIVE eq 'true' && $SERVICE_ACTIVE eq 'true'}
				<div class="btn-toolbar">
					<span class="btn-group">
						<button type="button" class="btn btn-default" id="addProduct1" data-module-name="Products" >

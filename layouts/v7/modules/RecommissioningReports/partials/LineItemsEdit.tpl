...
<input type="hidden" disabled="disabled" id="fildNamesOfCustPickFieldsInfo" value={ZEND_JSON::encode($LINEITEM_CUSTOM_PICK_FIELDS)}>
...
<div name='editContent' style="padding-top: 30px;">
	...
	{if $BLOCK_FIELDS|@count gt 0}
		<div class='fieldBlockContainer' id="PartsfieldBlockContainer">
			<div class="row">
				<div class="col-sm-3">
					<h4 class='fieldBlockHeader' style="margin-top:5px;">{vtranslate($ITEMBLOCKNAME, $MODULE)}</h4>
				</div>
			...
			<div class="lineitemTableContainer">
				<table class="table table-bordered" id="lineItemTab">
					...
					<tr id="row0" class="hide lineItemCloneCopy" data-row-num="0">
						{include file="partials/LineItemsContent.tpl"|@vtemplate_path:'RecommissioningReports' row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
					</tr>
					{if count($RELATED_PRODUCTS) gt 0}
						{foreach key=row_no item=data from=$RELATED_PRODUCTS}
							<tr id="row{$row_no}" data-row-num="{$row_no}" class="lineItemRow" {if $data["entityType$row_no"] eq 'Products'}data-quantity-in-stock={$data["qtyInStock$row_no"]}{/if}>
								{include file="partials/LineItemsContent.tpl"|@vtemplate_path:'RecommissioningReports' row_no=$row_no data=$data}
							</tr>
						{/foreach}
					{/if}
					{* {if count($RELATED_PRODUCTS) eq 0 and ($PRODUCT_ACTIVE eq 'true' || $SERVICE_ACTIVE eq 'true')}
						<tr id="row1" class="lineItemRow" data-row-num="1">
							{include file="partials/LineItemsContent.tpl"|@vtemplate_path:'RecommissioningReports' row_no=1 data=[] IGNORE_UI_REGISTRATION=false}
						</tr>
					{/if} *}
				</table>
			</div>

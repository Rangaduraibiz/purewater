{strip}
	...
	{if $SERVICEREPORTTYPE neq 'SERVICE FOR SPARES PURCHASED'}
	...
	{if $PRODUCT_EDITABLE}
				...
				<div class="row">
					<div class="col-lg-10">
						<div class="input-group" style="width:100%">
							<input type="text" id="{$productName_other}" name="{$productName_other}" value="{$data.$productName_other}" class="productName form-control autoComplete1" placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}"
								   data-rule-required=true {if !empty($data.$productName_other)} disabled="disabled" {/if}>
		...
		<td>
			{if $COMMENT_EDITABLE}
				<div><textarea id="{$comment}" name="{$comment}" class="lineItemCommentBox">{decode_html($data.$comment)}</textarea></div>
			{/if}
		</td>
	{/if}
	...
	{/if}
	<input type="hidden" id="fildNamesOfCustFieldsOther" value={ZEND_JSON::encode($LINEITEM_CUSTOM_FIELDNAMES_OTHER)}>
	...
	{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER}
		<td>
			...
			{if  $LINEITEM_CUSTOM_FIELD['uitype'] eq '5'}
				<div id="{$fieldName}DivCla"  {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
					<div class="input-group inputElement" style="margin-bottom: 3px">
						<input  id="{${$fieldName}}" name="{${$fieldName}}" type="text" class="form-control" data-fieldname="{${$fieldName}}" data-fieldtype="date" data-date-format="{$dateFormat}"
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '21' or $LINEITEM_CUSTOM_FIELD['uitype'] eq '2'}
				<div id="{$fieldName}DivCla"  {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
					<textarea id="{${$fieldName}}" name="{${$fieldName}}" class="">{decode_html($data.${$fieldName})}</textarea>
				</div>
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '7' }
				<div id="{$fieldName}DivCla" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
					<input id="{${$fieldName}}" name="{${$fieldName}}" value="{decode_html($data.${$fieldName})}" class="qty inputElement" data-fieldname="{$fieldName}" style="min-width: 100px;"></input>
				</div>
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '16'}
			    {if $LINEITEM_CUSTOM_FIELD['fieldname'] eq 'sad_line_status'}
					<select style="min-width: 150px;" id="{${$fieldName}}" class="select2-container select2 inputElement picklistfield lineitempicklistfield sad_line_status" name="{${$fieldName}}" data-fieldname="{$fieldName}" data-rownum="{$row_no}" data-fieldtype="picklist">
			    ...
			    {elseif $LINEITEM_CUSTOM_FIELD['fieldname'] eq 'sad_line_event'}
				    <select style="min-width: 150px;" id="{${$fieldName}}" class="select2-container select2 inputElement picklistfield lineitempicklistfield sad_line_event" name="{${$fieldName}}" data-fieldname="{$fieldName}" data-rownum="{$row_no}" data-fieldtype="picklist">
						<option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
						{foreach  key=PICKLIST_FIELDKEY item=PICKLIST_FIELD_ITEM from=$LINEITEM_CUSTOM_FIELD['picklistValues']}
							<option {if trim(decode_html($data.${$fieldName})) eq $PICKLIST_FIELDKEY} selected {/if} value="{$PICKLIST_FIELDKEY}">{$PICKLIST_FIELDKEY}</option>
						{/foreach}
				...
				{else}
					<div id="{$fieldName}DivCla" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
						<select data-rule-required=true style="min-width: 150px;" id="{${$fieldName}}" class="select2-container {if $row_no neq 0}select2{/if} inputElement picklistfield lineitempicklistfield" data-fieldname="{$fieldName}" data-rownum="{$row_no}" name="{${$fieldName}}" data-extraname="{$fieldName}" data-fieldtype="picklist">
							<option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
							{foreach  key=PICKLIST_FIELDKEY item=PICKLIST_FIELD_ITEM from=$LINEITEM_CUSTOM_FIELD['picklistValues']}
								<option {if trim(decode_html($data.${$fieldName})) eq $PICKLIST_FIELDKEY} selected {/if} value="{$PICKLIST_FIELDKEY}">{$PICKLIST_FIELDKEY}</option>
							{/foreach}

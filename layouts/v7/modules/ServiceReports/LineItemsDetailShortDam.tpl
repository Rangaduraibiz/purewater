...
{assign var=FINAL_DETAILS value=$RELATED_PRODUCTS_OTHER.1.final_details}
    ...
    <div class="lineItemTableDiv">
        <table class="table table-bordered lineItemsTable" style = "margin-top:15px">
            ...
            <th colspan="{$COL_SPAN3}" class="lineItemBlockHeader">
                {assign var=REGION_LABEL value=vtranslate('Shortages_And_Damages', $MODULE_NAME)}
                {if $RECORD->get('region_id') && $LINEITEM_FIELDS['region_id'] && $LINEITEM_FIELDS['region_id']->isViewable()}
                    ...
                    {/if}
                {/if}
                <h4>{vtranslate($ITEMBLOCKNAMEANOTHER, $MODULE)}</h4>
            </th>
            </thead>
            <tbody>
                <tr>
                ...
                {foreach key=INDEX item=LINE_ITEM_DETAIL from=$RELATED_PRODUCTS_OTHER}
                    <tr>
                        {if $IMAGE_VIEWABLE}
                            ...
                            <td>
                                <div>
                                    {if $LINE_ITEM_DETAIL["productDeleted$INDEX"]}
                                        {$LINE_ITEM_DETAIL["productName_other$INDEX"]}
                                    {else}
                                        <h5><a class="fieldValue" href="index.php?module={$LINE_ITEM_DETAIL["entityType$INDEX"]}&view=Detail&record={$LINE_ITEM_DETAIL["hdnProductId$INDEX"]}" target="_blank">{$LINE_ITEM_DETAIL["productName_other$INDEX"]}</a></h5>
                                        {/if}
                                </div>
                                {if $LINE_ITEM_DETAIL["productDeleted$INDEX"]}
                                    <div class="redColor deletedItem">
                                        {if empty($LINE_ITEM_DETAIL["productName_other$INDEX"])}
                                ...
                                {if $COMMENT_VIEWABLE && !empty($LINE_ITEM_DETAIL["productName_other$INDEX"])}
                                    <div>
                        ...
                        {if $QUANTITY_VIEWABLE}
                            <td>
                                {$LINE_ITEM_DETAIL["qty_other$INDEX"]}
                            </td>
                        {/if}
                        {foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER}

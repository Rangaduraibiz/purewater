	...
	{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS}
		<td>
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '7' }
				<div id="DivCla{$fieldName}" {if $LINEITEM_CUSTOM_FIELD["hiddenSpecial"] eq true} class="hide" {/if}>
					<input  {if $LINEITEM_CUSTOM_FIELD["hiddenSpecial"] eq true} type="hiden" {/if} {if $LINEITEM_CUSTOM_FIELD["isMandatory"] eq true} data-rule-required="true" {/if} id="{${$fieldName}}" name="{${$fieldName}}" value="{decode_html($data.${$fieldName})}" class="inputElement {$fieldName} {$data.$productName}" data-extraname="{$fieldName}" style="min-width: 100px;">
				</div>
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '16'}
				<select {if $LINEITEM_CUSTOM_FIELD["isMandatory"] eq true} data-rule-required="true" {/if} style="min-width: 150px;" id="{${$fieldName}}" class="select2-container inputElement select2 inputElement picklistfield" name="{${$fieldName}}" class="inputElement picklistfield" data-fieldtype="picklist">

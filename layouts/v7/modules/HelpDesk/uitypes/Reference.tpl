...
<div class="referencefield-wrapper {if $FIELD_VALUE neq 0} selected {/if}">
    ...
    <div class="input-group">
        {if $FIELD_MODEL->getFieldName() eq 'func_loc_id' || $FIELD_MODEL->getFieldName() eq 'parent_id'}
            ...
            <input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="reference" type="text" 
                ...
                readonly
                />
                 {* <a href="#" class="clearReferenceSelection refField{$FIELD_NAME} {if $FIELD_VALUE eq 0}hide{/if}"> x </a>
                 <span class="input-group-addon relatedPopup cursorPointer refField{$FIELD_NAME} hide" title="{vtranslate('LBL_SELECT', $MODULE)}">
            ...
            {* {if (($smarty.request.view eq 'Edit') or ($MODULE_NAME eq 'Webforms')) && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES)}
                <span class="input-group-addon createReferenceRecord cursorPointer clearfix refField{$FIELD_NAME} hide" title="{vtranslate('LBL_CREATE', $MODULE)}">
         ...
         {else}
            ...
            <input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="reference" type="text" 
                ...
                />
                <a href="#" class="clearReferenceSelection refField{$FIELD_NAME} {if $FIELD_VALUE eq 0}hide{/if}"> x </a>
                <span class="input-group-addon relatedPopup cursorPointer refField{$FIELD_NAME}" title="{vtranslate('LBL_SELECT', $MODULE)}">
                    <i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_select" class="fa fa-search"></i>
                </span>
            {* {if (($smarty.request.view eq 'Edit') or ($MODULE_NAME eq 'Webforms')) && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES)}
                <span class="input-group-addon createReferenceRecord cursorPointer clearfix refField{$FIELD_NAME}" title="{vtranslate('LBL_CREATE', $MODULE)}">
                    <i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_create" class="fa fa-plus"></i>
                </span>
            {/if} *}
         {/if}
